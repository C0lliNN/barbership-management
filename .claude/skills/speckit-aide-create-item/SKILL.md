...
Before drafting, read the **confirmed/pinned stack** in `docs/aide/vision.md` (§5.1)
and the constitution (`.specify/memory/constitution.md`) if filled in.

- **Align to pinned choices** (language, frameworks, libraries) — do NOT re-propose
  alternatives in the work item's Decisions section for anything already pinned.
- If the item genuinely needs a **new** library/framework not yet pinned, surface it
  as an **open decision to confirm with the user BEFORE implementation** (not a
  default-to-confirm, and never a post-completion swap). Note it explicitly in the
  item so it is resolved during `create-item`, not `execute-item`.
- Routine, item-local choices (JSON shapes, middleware order, etc.) stay in the
  item's Decisions log as before.
